...
def run_xgb_experiment(config):
    ...
    if "top20" in config["run_feature_sets"]:
        feature_sets["xgb_top20"] = top20_features
    if "all_features" in config["run_feature_sets"]:
        feature_sets["xgb_all_features"] = all_features
    ...
    for exp_name, features, weight in experiments:
        ...
        imp["f_index"] = [f"f{i}" for i in range(len(features))]
        imp["importance_gain"] = imp["f_index"].map(gain_scores).fillna(0.0)
        imp["importance_weight"] = imp["f_index"].map(weight_scores).fillna(0.0)
        ...
        importance_parts.append(
            imp[["experiment", "feature", "importance_gain", "importance_weight", "rank_gain"]]
        )
    ...
    valid_result["xgb_probability"] = pred_dict[best_exp]["valid_prob"]

    test_result = test_df[config["id_columns"]].copy()
    test_result["xgb_probability"] = pred_dict[best_exp]["test_prob"]

    prediction_result = pd.concat([valid_result, test_result], ignore_index=True)

    summary_path = os.path.join(output_dir, "xgb_experiment_summary_with_path.csv")
    metrics_path = os.path.join(output_dir, "xgb_model_metrics_with_path.csv")
    top_path = os.path.join(output_dir, "xgb_top_rate_metrics_with_path.csv")
    importance_path = os.path.join(output_dir, "xgb_feature_importance_with_path.csv")
    pred_path = os.path.join(output_dir, "xgb_predictions_best_with_path.csv")
    model_path = os.path.join(output_dir, f"best_xgb_model_{best_exp}.joblib")
    features_path = os.path.join(output_dir, f"best_xgb_features_{best_exp}.csv")
    ...
    pd.DataFrame({"feature": best_features}).to_csv(features_path, index=False, encoding="utf-8-sig")
